...
function descobrirIdade() {
    let data = document.getElementById("inputData").value; // Alterado para let
    data = new Date(data).getFullYear();
    let idade = 2024 - data;
    return idade;
}
...
function categorizar(idade) {
    ...
    if (idade === 12 || idade === 13) {
        ...
        categ = "Categoria Sub-20";
    } else if (idade >= 20 && idade <= 22) { // Corrigido para idade >= 20
        categ = "Categoria Sub-23";
    } else if (idade >= 40) {
        categ = "Categoria de Masters";
    } else {
        categ = "Categoria de Adultos"; // Mover aqui
    }
...
function descobrirGenero() {
    let genero; // Declarada a variável
    let g = document.getElementById("inputGenero").value;
    if (g === "fem") {
        genero = "Feminino";
    } else {
...
function descobrirModalidade() {
    let modalidade; // Declarada a variável
    let M = document.getElementById("inputModalidade").value;
    if (M === "inputCV") {
        modalidade = "Corrida de Velocidade";
    } else if (M === "inputCR") {
        modalidade = "Corrida de Resistência";
    } else if (M === "inputLA") {
        modalidade = "Lançamento ou Arremesso";
    } else {
        modalidade = "Salto";
    }
    return modalidade; // Adicionada para retornar o valor
}
...
function salvar() {
    ...
    const nome = document.getElementById("inputNome").value;
    const idade = descobrirIdade(); // Chamando a função para obter a idade
    const modalidade = descobrirModalidade(); // Chamando a função para obter a modalidade
    const genero = descobrirGenero(); // Chamando a função para obter o gênero
    const competicoes = parseInt(document.getElementById("inputCompeticoes").value);
    const medalhas = parseInt(document.getElementById("inputMedalhas").value);

    // Verificar se o que foi digitado pelo USUÁRIO está correto
    if (id && nome && idade && medalhas && modalidade && genero && competicoes) { // se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                atleta = new Atleta(id, nome, idade, medalhas, modalidade, genero, competicoes); // Usando Atleta com letra maiúscula
                listaDeAtletas.push(atleta);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                atletaAlterado = new Atleta(id, nome, idade, medalhas, modalidade, genero, competicoes);
                listaDeAtletas[atleta.posicaoNaLista] = atletaAlterado;
                mostrarAviso("Alterado");
                break;
            case 'excluindo':
...
function inserirDadosIniciais() {
    ...
    listaDeAtletas = [];//se houver dados na lista, apaga todos
    let atleta = new Atleta('111', 'Ana Silva', 14, "Categoria Sub-16", "11/01/1111",);
    listaDeAtletas.push(atleta);
    atleta = new Atleta('222', 'Bruno Costa', 12, "Categoria Sub-14", "11/01/1111");
    listaDeAtletas.push(atleta);
    atleta = new Atleta('333', 'Carla Oliveira', 16, "Categoria Sub-18","11/01/1111");
    listaDeAtletas.push(atleta);
    atleta = new Atleta('444', 'Daniel Souza', 14,"Categoria Sub-16","11/01/1111");
    listaDeAtletas.push(atleta);
    atleta = new Atleta('555', 'Eduardo Lima', 25, "Categoria Masters" ,"11/01/1111");
    listaDeAtletas.push(atleta);
    atleta = new Atleta('666', 'Bruno Costa', 19, "Categoria Sub-20","11/01/1111");
    listaDeAtletas.push(atleta);
    atleta = new Atleta('777', 'João Lucas', 15, "Categoria Sub-16","11/01/1111");
    listaDeAtletas.push(atleta);
    listar();
    visibilidadeDosBotoes('inline', 'none', 'none', 'none', 'none');
    bloquearAtributos(true);
